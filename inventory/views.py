...
def item_list(request):
    ...
    # Evaluate products once, then load all branch quantities in one query.
    ...
    if current_branch and items:
        branch_stocks = (
            BranchStock.objects
            .filter(
                branch=current_branch,
                variant__item_id__in=[item.id for item in items],
            )
            .values("variant_id", "variant__item_id", "quantity")
        )

        for stock in branch_stocks:
            item_id = stock["variant__item_id"]
            variant_id = stock["variant_id"]
            quantity = int(stock["quantity"] or 0)

            item_stock_map[item_id] = item_stock_map.get(item_id, 0) + quantity
            variant_stock_map[variant_id] = quantity
...
def variant_barcode_label(request, variant_id):
    ...
    return render(request, "inventory/barcode_label.html", {
        ...
        "barcode_return_url": request.GET.get("next") or "/inventory/",
    })
